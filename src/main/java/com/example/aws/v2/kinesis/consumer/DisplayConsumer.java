...
public class DisplayConsumer implements ShardRecordProcessor {
	...
	private static final boolean IF_TIME_FIELD_ENABLED = true;
	private static final String TIME_FIELD_NAME = "time";
	private static final String TIME_FIELD_FORMAT = "yyyy-MM-dd'T'HH:mm:ss.SSSxxxxx";
	...
	public void processRecords(ProcessRecordsInput processRecordsInput) {
		...
		try {
			...
			//List<KinesisClientRecord> kinesisClientRecordList = processRecordsInput.records();
			/*for(KinesisClientRecord record: kinesisClientRecordList) {
				processSingleRecord(record);
			}*/
		} catch (Throwable t) {
			LOG.error("Caught throwable while processing records.  Aborting" +t.toString());
	 ...
	 */
	private void processSingleRecord(KinesisClientRecord record) throws CharacterCodingException {
		String data = null;
		LOG.info("Processing..............");
		data = decoder.decode(record.data()).toString();

		System.out.println("Shard: " + shardId + ", PartitionKey: " + record.partitionKey() + ", SequenceNumber: "
				+ record.sequenceNumber() + " Arrived "  + "data" + data);
